# flexorm
flexorm is a Java library for relational database access. 
...
# Maven Setting
flexorm is available in maven central repo.
```xml
<dependency>
    <groupId>io.github.dynamixon</groupId>
...
dummies = queryEntry.findObjects(Dummy.class, new Cond.Builder().columnName("id").compareOpr("is not null").ignoreNull(false).build());
```
Corresponding SQLs:
```sql
...
**`NOTE:`** The single `batchInsert` operation below is separated into two insert queries. flexorm will automatically group together those records which can be operated in a single query. The reason for this seemingly odd behaviour is that we must let database decide how to deal with the null values, `dummy3` here doesn't have `varcharF` set, if there is only one insert query, it would look like this: `insert into dummy (int_f,varchar_f) values (?,?) , ( ?,?), ( ?,?) -- values: [10, "some text", 20, "other text", 30, null]`. Note the last value is null, if `varchar_f` has a default value, the default value can never be applied. 
```java
...
### persist
To persist (same as upsert if you are more familiar with this term) a record into database, instantiate a Java object and fill it with data, specify the conditions to filter out the record to be updated if it ever exists, then call `QueryEntry.persist()`. flexorm will first try to perform an update operation by the conditions, if no database record is updated, the object will be inserted into database instead.
...
### SQL identify
Due to flexorm's nature of constructing SQL at runtime, SQL query can be hard to trace back to, by adding a comment as its identity, tracing will no longer be as hard. 
...
Manually compose Java Bean files from database table definition can be both tedious and error prone, flexorm provides a small tool to take over this job.

Place a file named "Table2JavaMeta.json" (see below) inside your project's root directory, execute the main method inside `Table2Java.java`, and the Java files will be generated at this location: \[your project root/\<srcRoot\>/\<domainPackage\>\]. Based on the example json below, a file named "Dummy.java" can be located at "your project root/src/test/java/io/github/dynamixon/dataobject/Dummy.java" after a successful generation.
...
```json
{
  ...
  "domainPackage":"io.github.dynamixon.dataobject",
...
### transaction support
By wrapping your dataSource in a `TransactionAwareDataSourceProxy`, flexorm can fully support Spring's Transaction Management.
