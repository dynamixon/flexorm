# flexorm
flexorm 是一个可以用来操作关系型数据库的Java工具库.
...
**`注意:`** 下面代码示例的一个单次`batchInsert`调用会被分成两个SQL. flexorm会自动聚合可以在同一个语句中进行插入的数据. 这个看似奇怪的行为其实是因为针对null值的字段我们必须交给数据库自身去处理, 这里的`dummy3`里字段`varcharF`的值没设置, 如果只有一个SQL语句的话就会变成这种: ` insert into dummy (int_f,varchar_f) values (?,?) , ( ?,?), ( ?,?) -- values: [10, "some text", 20, "other text", 30, null]`. 注意最后一个值是null, 如果`varchar_f`有默认值的话那这个默认值就没法被应用了.
```java
...
### 持久化
要将一个数据持久化(或者叫更新插入)到数据库中, 实例化一个Java对象并填充数据, 指定更新的条件, 然后调用`QueryEntry.persist()`. flexorm会首先尝试根据指定的条件更新, 如果没有数据最终被更新, 则执行插入操作.
...
### sql标注
由于flexorm本身会在运行时才组装SQL, 就不容易追查SQL语句的来源, 如果在语句前面加一个身份标签那就可以方便追查了. 
...
### Java文件生成
根据表结构定义手工编写Java文件即麻烦又容易出错, flexorm提供了一个小工具来处理这部分工作.

将一个名为"Table2JavaMeta.json"的文件(参考下面)房子在工程主目录下面, 执行`Table2Java.java`里的main方法, Java文件就会在这个位置创建: \[your project root/\<srcRoot\>/\<domainPackage\>\]. 以下面的json为例, 生成成功后一个名为"Dummy.java"的文件会被放置在这里:"your project root/src/test/java/io/github/dynamixon/dataobject/Dummy.java".
...
### 事务支持
将数据源对象用`TransactionAwareDataSourceProxy`包装, flexorm 即可支持Spring的事务管理.
